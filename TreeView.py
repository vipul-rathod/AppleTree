    ...
    def _getMovFiles(self):
        ...
        for root, dirs, files in os.walk(self.shFldPath):
            for fl in sorted(files):
                if fl.lower().endswith('.mov'):
                    treeWidgetItem = QtGui.QTreeWidgetItem(['%s.%s' % (fl.split('.')[0], fl.split('.')[1])])
    ...
    def _moveFilesToPublish(self):
        ...
        if selectedShots:
            for each in selectedShots:
                ...
                for root, dirs, files in os.walk(self.shFldPath):
                    for fl in sorted(files):
#                         print ('%s.%s' % (each, fl.split('.')[-1].lower()))
                        if fl.lower().endswith('.mov') and fl == ('%s.%s' % (each, fl.split('.')[-1].lower())):
#                             print "Step01"
                            ...
                            while os.path.exists(os.path.join(self.publishPath, fl)):
                                ...
                                if allFiles:
                                    for allFile in allFiles:
                                        if allFile.lower().endswith('.mov'):
                                            print "Step02"
                                            publishFiles.append(allFile)
                                versionNumber = int(sorted(publishFiles)[-1].split('.v')[1].split('.')[0])
                                versionNumber += 1
                                print "Step03"
                                if versionNumber < 10:
